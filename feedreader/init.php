...
function fof_is_subscribed($url)
{
   ...
   $result = fof_do_query("select url, title, link, id from feeds where url LIKE  '%$safe_url%' ");
...
//  update feed used in mepath daily //////////////////////////////////////////////////
...
//   	echo "Memory usage after set cache: " . number_format(memory_get_usage()); 
  ...
  $piefeed->handle_content_type();
   ...
   if (!$piefeed->data)
   {
      ...
      //unset($piefeed);
         $piefeed->__destruct(); // Do what PHP should be doing on it's own.
